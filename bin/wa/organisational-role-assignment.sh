...
ROLE_NAME="${4:-"ctsc"}"
ROLE_ATTRIBUTES="${5:-'{"jurisdiction":"PROBATE"}'}"
...
USER_TOKEN=$($BASEDIR/bin/idam-lease-user-token.sh $USERNAME $PASSWORD)
USER_ID=$($BASEDIR/bin/idam-user-id.sh $USER_TOKEN)
SERVICE_TOKEN=$($BASEDIR/bin/idam-lease-service-token.sh prl_cos_api \
