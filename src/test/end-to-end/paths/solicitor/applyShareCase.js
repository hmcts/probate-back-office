...
Scenario(scenarioName, async function ({I}) {
        ...
        await I.cyaPage();
        const caseRef = await I.grabTextFrom('//div[@class="column-one-half"]//ccd-case-header');
        const caseIdShareCase = caseRef.replace(/#/g, '')
        const caseRefNumber = caseRef.replace(/\D/g, '');
        await I.shareCaseSelection(caseRefNumber);
        await I.logInfo(scenarioName, 'Login as PP user 2');
        await I.authenticateUserShareCase(false);
        await I.verifyShareCase(caseRefNumber);
        await I.logInfo(scenarioName, 'Login as PP user 1');
        await I.logInfo(scenarioName, 'Verify Case ' + caseRef + ' is not shared with  PP user 1');
        await I.authenticateUserShareCase(true);
        await I.shareCaseVerifyUserRemove(caseRefNumber);
        await I.logInfo(scenarioName, 'Login as PP user 2');
        await I.logInfo(scenarioName, 'Delete the case ' + caseRef + ' and sign out');
        await I.authenticateUserShareCase(false);
        await I.shareCaseDelete(caseIdShareCase,caseRef);
