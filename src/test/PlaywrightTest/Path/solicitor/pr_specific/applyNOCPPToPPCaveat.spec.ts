...
test.describe("Solicitor - Apply NOC Caveat", () => {
  test("Solicitor - Apply Caveat", async ({
    ...
    completeApplicationConfig.caveat_expiry_date = dateFns.format(dateFns.addMonths(new Date(), 6), testConfig.dateFormat);
    // When emailing the caveator, the Date added for the email document is set to today
    completeApplicationConfig.notification_date = dateFns.format(new Date(), testConfig.dateFormat);
    ...
    await solCreateCasePage.nocPage2(deceasedDetailsConfig.page2_deceased_surname);
    await solCreateCasePage.nocPage3(caseRef, deceasedDetailsConfig.page2_deceased_surname);
    ...
    if (env === 'Demo') {
      ...
      changeOfRepresentativesCaveatorDetailsConfigDemo.nocTriggeredDate = dateFns.format(new Date(), testConfig.dateFormat);
