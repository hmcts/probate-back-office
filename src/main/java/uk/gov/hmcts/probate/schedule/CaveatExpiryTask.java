...
public class CaveatExpiryTask implements Runnable {
    ...
    public void run() {
        ...
        String expiryDate = DATE_FORMAT.format(LocalDate.now(clock).minusDays(1L));
        if (StringUtils.isNotEmpty(adHocJobDate)) {
            log.info("Running CaveatExpiryTask with Adhoc date");
            expiryDate = adHocJobDate;
        } else {
            log.info("Running CaveatExpiryTask with default date");
        }
        ...
        try {
            ...
            log.error("API client exception during CaveatExpiryTask", e);
        } catch (Exception e) {
            log.error("Error on CaveatExpiryTask Scheduler", e);
        }
    }
}
