...
public class BulkPrintService {
    ...
    public SendLetterResponse sendToBulkPrintForCaveat(CaveatCallbackRequest caveatCallbackRequest,
    ...
    public SendLetterResponse sendDocumentsForReprint(CallbackRequest callbackRequest, Document selectedDocument,
                                                      Document coverSheet) {
    ...
    private SendLetterResponse sendToBulkPrint(CallbackRequest callbackRequest, Document grantDocument,
        ...
        try {
            ...
            List<uk.gov.hmcts.reform.sendletter.api.model.v3.Document> pdfs;
            if (forReprint) {
                pdfs = arrangePdfDocumentsForReprint(callbackRequest, grantDocument, coverSheet);
            } else {
    ...
    private List<uk.gov.hmcts.reform.sendletter.api.model.v3.Document> arrangePdfDocumentsForBulkPrinting(
        CallbackRequest callbackRequest,
        Document grantDocument,
        Document coverSheetDocument) throws IOException {
        Long extraCopies = 1L;
        List<uk.gov.hmcts.reform.sendletter.api.model.v3.Document> documents = new LinkedList<>();
        String encodedCoverSheet = getPdfAsBase64EncodedString(coverSheetDocument, callbackRequest);
        String encodedGrantDocument = getPdfAsBase64EncodedString(grantDocument, callbackRequest);
        Set<DocumentType> documentTypes = Set.of(
                DIGITAL_GRANT, ADMON_WILL_GRANT, INTESTACY_GRANT, AD_COLLIGENDA_BONA_GRANT,
                WELSH_DIGITAL_GRANT, WELSH_INTESTACY_GRANT, WELSH_ADMON_WILL_GRANT,
                WELSH_AD_COLLIGENDA_BONA_GRANT, INTESTACY_GRANT_REISSUE, ADMON_WILL_GRANT_REISSUE,
                DIGITAL_GRANT_REISSUE, AD_COLLIGENDA_BONA_GRANT_REISSUE, WELSH_DIGITAL_GRANT_REISSUE,
                WELSH_ADMON_WILL_GRANT_REISSUE, WELSH_INTESTACY_GRANT_REISSUE,
                WELSH_AD_COLLIGENDA_BONA_GRANT_REISSUE
        );

        boolean hasDocumentType = documentTypes.stream()
                .anyMatch(documentType -> documentTransformer.hasDocumentWithType(Arrays.asList(grantDocument),
                        documentType));
    ...
    private List<uk.gov.hmcts.reform.sendletter.api.model.v3.Document> arrangePdfDocumentsForBulkPrinting(
        ...
        String encodedCoverSheet =
            getPdfAsBase64EncodedString(coverSheetDocument, caveatCallbackRequest);
        String encodedGrantDocument =
            getPdfAsBase64EncodedString(grantDocument, caveatCallbackRequest);

        uk.gov.hmcts.reform.sendletter.api.model.v3.Document coversheetDocument =
            new uk.gov.hmcts.reform.sendletter.api.model.v3.Document(encodedCoverSheet, 1);
        uk.gov.hmcts.reform.sendletter.api.model.v3.Document document =
            new uk.gov.hmcts.reform.sendletter.api.model.v3.Document(encodedGrantDocument, 1);
    ...
    private List<uk.gov.hmcts.reform.sendletter.api.model.v3.Document> arrangePdfDocumentsForReprint(
        CallbackRequest callbackRequest,
        Document grantDocument,
        Document coverSheetDocument) throws IOException {
