...
public class ExceptionRecordCaseDataValidator {
    ...
    public static void validateScannedDocumentTypes(List<CollectionMember<ScannedDocument>> scannedDocuments,CaseType caseType){
        List<String> disallowedDocTypesFound =
                scannedDocuments
                        .stream()
                        .filter(item -> allowScannedDocumentTypes.containsKey(item.getValue().getType())
                                && !allowScannedDocumentTypes.get(item.getValue().getType()).contains(caseType))
                        .map(item -> item.getValue().getType())
                        .collect(toList());

        if (!disallowedDocTypesFound.isEmpty()) {

            String errorMessage = String.format(
                    INVALID_SCANNED_DOCUMENT_TYPE_ERROR, caseType,
