...
public class PA1PCitizenMandatoryFieldsValidator {
    ...
    protected void addWarningsFormVersion2(HashMap<String, String> ocrFieldValues, ArrayList<String> warnings) {
        Stream.of(GORCitizenMandatoryFields.values()).filter(field -> field.isVersion2()).forEach(field -> {
            ...
            if (!ocrFieldValues.containsKey(field.getKey())) {
                log.warn("v2 {} was not found in ocr fields", field.getKey());
                warnings.add(format(MANDATORY_FIELD_WARNING_STIRNG, field.getValue(), field.getKey()));
            }
        });
        ...
        if (deceasedDiedOnAfterSwitchDate) {
            mandatoryFieldsValidatorUtils.addWarningsForConditionalFields(warnings, ocrFieldValues, IHT_ESTATE_GROSS, 
        ...
        if (iht205completedOnline) {
            mandatoryFieldsValidatorUtils.addWarningsForConditionalFields(warnings, ocrFieldValues, IHT_IDENTIFIER);
        }
    }
