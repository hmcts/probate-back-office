...
public class AssignCaseAccessService {
    ...
    public void assignCaseAccessCaveat(CaveatDetails caveatDetails, String authorisationToken) {
        ResponseEntity<Map<String, Object>> userResponse = idamApi.getUserDetails(authorisationToken);
        Map<String, Object> result = Objects.requireNonNull(userResponse.getBody());
        String userId = result.get("id").toString().toLowerCase();

        log.info("CaseId: {} assigning case access to user {}", caveatDetails.getId(), userId);
        ...
        assignCaseAccessClient.assignCaseAccess(
            ...
            serviceToken,
            true,
            buildAssignCaseAccessRequest(caveatDetails, userId)
        );
        ccdDataStoreService.removeCreatorRoleCaveat(caveatDetails, authorisationToken);

        log.info("CaseId: {} assigned case access to user {}", caveatDetails.getId(), userId);
    }

    private AssignCaseAccessRequest buildAssignCaseAccessRequest(CaveatDetails caveatDetails, String userId) {
        return AssignCaseAccessRequest
            .builder()
            .caseId(caveatDetails.getId().toString())
            .assigneeId(userId)
            .caseTypeId(CAVEAT.getName())
            .build();
    }
}