...
@Slf4j
@AllArgsConstructor
public class BulkPrintService2 {
    ...
    private List<PrintDocument> arrangePdfDocumentsForBulkPrinting(
        ...
        log.info("Grant caseId {} coverSheetDocument dm store {}",
                caseId,
                coverSheetDocument.getDocumentFileName());
        byte[] coverBytes = documentStoreClient.retrieveDocument(coverSheetDocument, authHeaderValue);

        log.info("Grant caseId {} grantDocument dm store {}",
                caseId,
                grantDocument.getDocumentFileName());
        byte[] grantBytes = documentStoreClient.retrieveDocument(grantDocument, authHeaderValue);
    ...
    private List<PrintDocument> arrangePdfDocumentsForBulkPrinting(
        ...
        log.info("Caveat caseId {} coverSheetDocument dm store {}",
                caseId,
                coverSheetDocument.getDocumentFileName());
        byte[] coverBytes = documentStoreClient.retrieveDocument(coverSheetDocument, authHeaderValue);

        log.info("Caveat caseId {} grantDocument dm store {}",
                caseId,
                grantDocument.getDocumentFileName());
        byte[] grantBytes = documentStoreClient.retrieveDocument(grantDocument, authHeaderValue);
    ...
    private List<PrintDocument> arrangePdfDocumentsForReprint(
        ...
        byte[] coverBytes = getPdfAsBytes(coverSheetDocument, authHeaderValue, callbackRequest);
        byte[] grantBytes = getPdfAsBytes(grantDocument, authHeaderValue, callbackRequest);
    ...
    private byte[] getPdfAsBytes(Document document,
            String authHeaderValue,
            CallbackRequest callbackRequest) throws IOException {

        log.info("caseId {} dm store {}",
